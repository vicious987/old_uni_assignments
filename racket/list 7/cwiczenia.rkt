...
(define (mylist-head x)
  (pair-fst x))

(define (mylist-tail x)
  (pair-snd x))

(define (mylist-cons lst)
  (mylist-cons_ lst mynull))

(define (mylist-cons_ lst acc)
  (if (empty? lst acc)
      acc
      (mylist-cons_ (pair-cons (car lst) acc))))

(define (make-list2 lst env)
  (if (empty? lst)
      mynull
      (cons (eval-env (car lst) env) (make-list2 (cdr lst) env))))
...
(define (eval-env e env)
  (cond [(const? e) e]
        [(mynull? e) mynull]
        [(mynull?? e) (eq? mynull (eval (second e)))]
        [(mylist? e) (eval-env (mylist-head e))
        [(op? e)
