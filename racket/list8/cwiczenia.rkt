...
(define (lambda-multi? e)
  (and (tagged-tuple? 'lambda 3 e)
       (symbol? (second e))))

(define (lambda-multi-cons varlist body)
...
(define (closure-multi? c)
  (tagged-tuple? 'closure-multi 4 c))

(define (closure-multi-vars c)
  (third c))
  ...
  (cond [(const? e)
        ...
        [(lambda-rec? e)
         (closure-rec-cons (lambda-rec-name e)
                           ...
                           env)]
        [(lambda-mult? e)
         (closure-cons (cons 'lambda ((x) (lambda-mult-body e) env)]
        [(app? e)
         (apply-closure
...
#|
(let (myappend (lambda-rec (app xs ys)
                           (if (null? xs)
                             ys
...
#|
(let (mymap (lambda-rec (maper proc lst)
                        (if (null? xs)
                            null
                            (cons (proc (car xs))
                                  (maper proc (cdr xs)))))))
