...
(eval 3)
(eval '((lambda (x) (+ x 1)) 5))
(eval '(let (inc (lambda (x) (+ x 1))) (inc 9)))

(eval '(const? (let (x 3) x)))
(eval '(+ 3 5))
(eval '(op? (+ 3 5)))
...
#|
(eval '(
(let (arith-exp? (lambda-rec (ae? x)
                             (cond
                               [(or (const? x) (var? x)) true]
                               [(and (op? x) (ae? (car (cdr x)) (ae? (car (cdr (cdr x))))))]
                               [true false])))
  (let (evaluator (lambda-rec (my_eval e)
                              (cond
                                [(const? e) e]
                                [(var? e) e]
                                [(arith-exp? e) (cons (car e) (cons (my_eval (car (cdr e))) (cons (my_eval (car (cdr (cdr e)))) null)))]))))
  )
