#lang racket
;; Mateusz Obara pracownia nr 14
;; dodana klasa duck dziedzicząca po person zaczyna się w lini 296
...
;; nowe - dziedziczy po person, rozszerzona o metode 'make noise' i 'waddle'
(define (make-duck name birthplace threshold)
  (let ((person (make-person name birthplace threshold)))
    (lambda (message)
     (cond ((eq? message 'act)
            (lambda (self)
           ...
           ((eq? message 'waddle)
            (lambda (self)
              (newline)
              (display-message (list name "robi kółeczko dreptając"))))
           ((eq? message 'make-noise)
	    (lambda (self)
               (newline)
               (display-message (list name "rozgląda się, a następnie wydaje z siebie przerażający dźwięk"))
	       '*KWA*))
	    (else (get-method person message))))))

(define (make&install-duck name birthplace threshold)
  (let ((duck  (make-duck name birthplace threshold)))
    (ask duck 'install)
    duck))
...
(define automaty (make-place 'automaty)) ;nowe
(define biblioteka (make-place 'biblioteka)) ;nowe
(define dziekanat (make-place 'dziekanat)) ;nowe
...
(can-go-both-ways schody-parter 'zachód 'wschód biblioteka) ;nowe
...
(can-go-both-ways piętro-wschód 'wschód 'zachód automaty) ;nowe
...
(can-go-both-ways wielka-wschodnia 'południe 'północ dziekanat) ;nowe
;; Osoby dramatu
;;---------------------------------------
...
(define miła-pani-z-dziekanatu (make&install-person 'miła-pani-z-dziekanatu dziekanat 3)) ;nowe
(define donald-the-duck (make&install-duck 'donald-the-duck hol 2)) ;nowe
...
(define porzucona-deklaracja (make&install-thing 'porzucona-deklaracja hol)) ;nowe
...
(define jokebook (list
               '("what do you call a snake who works in the government? a civil serpent")
               '("how many tickles does it take to make octopus laugh? ten-tickles")
...
(define (check-inventory) ;nowe
  (ask *player-character* 'list-possessions)
  #t)

(define (speak words) ;nowe // komunikat musi być otoczonym cudzysłowem
  (ask *player-character* 'say '(words))
  #t)
  
(define (joke) ;nowe
  (let ((j (pick-random jokebook)))
    (ask *player-character* 'say j)
    #t))
       
(define (get-angry) ;nowe
  (ask *player-character* 'have-fit)
  #t)

(define (pick-up stuff) ;nowe
  (ask *player-character* 'take stuff)
  #t)

(define (throw-out stuff) ;nowe
  (ask *player-character* 'lose stuff)
  #t)

(define (move-to location) ;nowe
  (ask *player-character* 'move-to location)
  #t)
